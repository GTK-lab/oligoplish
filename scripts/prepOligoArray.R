...
txdb <- read_tsv(here(hg38_tss_fn),skip=1,col_types=cols(),
    ...
    mutate(strand=fct_recode(factor(strand),"+"="1","-"="-1"),
           seqname=sprintf('chr%s',seqname),
           start=tss,end=tss)

if (any(syms %in% txdb$symbol)) {
    cat(sprintf("NOT FOUND: '%s'\n",paste(syms[!(syms %in% txdb$symbol)],sep=",")))
}
syms <- syms[syms %in% txdb$symbol]
cat(sprintf("WORKING ON: '%s'\n",paste(syms,sep=",")))

outdir <- sprintf("%s",project)
cat(sprintf("results stored in %s\n",outdir))
dir.create(outdir,showWarnings = FALSE,recursive = TRUE)

writeLines(syms,sprintf("%s/symbols.txt",outdir))
